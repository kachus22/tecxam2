...
export class QuestionsComponent implements OnInit {
  ...
  load(){
    this.loading = true;
    this.questionsService.fill2()
      .subscribe(
        (result) => {
          this.showSuccess('Preguntas cargadas!');
          this.loading = false;
          this.rows = [];
          for(var i in result){
  ...
  updateQuestion(){
    ...
    this.questionsService.update2(this.questionEdit.id, question)
      .subscribe(
        (result) => {
  ...
  onDelete(){
    this.questionsService.delete2(this.questionEdit.id)
        .subscribe(
          (result) => {
  ...
  add(postBody: any){
    this.questionsService.add2(postBody)
      .subscribe(
        (result) => {
          this.load();
